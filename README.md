...
- `Hermes-Windows-Launcher-v2026.05.04.20.zip`: versioned Windows download linked by `index.html` (任务 013 hotfix：修复 Stop-ExistingGateway 漏杀 venv python worker 导致 gateway.lock 不释放 → 新 gateway 自杀 → webui "未连接"。陷阱 #39：venv python.exe 是 stub launcher，必须用 `Get-CimInstance Win32_Process` 按 CommandLine 过滤，不能用 `Get-Process.Path`)
